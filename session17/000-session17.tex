...
\hline
\textbf{Service} & \textbf{Action} & \textbf{Path} & \textbf{Argument} & \textbf{Description} \\
\hline
PING & \texttt{ping} & \texttt{/ping} & none & Test if server is alive \\
\hline
GET & \texttt{get} & \texttt{/get} & \texttt{n}: 0--4 & Get sequence number n \\
\hline
GENE & \texttt{gene} & \texttt{/gene} & \texttt{name} & Get complete gene sequence \\
\hline
OPERATION & \texttt{operation} & \texttt{/operation} & \texttt{seq}, \texttt{op} & Perform operation on sequence \\
\hline
\end{tabular}
\end{center}
\endgroup
