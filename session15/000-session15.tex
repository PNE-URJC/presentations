...
class Bases2Handler(
    ...
    def do_GET(self):
        ...
        # 3. Send response with send_response(),
        #    send_header(), end_headers()
        # 4. Write body with self.wfile.write()
        pass
...
\begin{frame}[fragile]
\frametitle{Handling the Path and Serving Files}

Use \pythoninline{self.path} to determine which file to serve:
...
def do_GET(self):
    # 1. Map "/" to "/index.html"
    #    Otherwise build path: "html" + self.path

    # 2. Try to open the file (Path.read_text())
